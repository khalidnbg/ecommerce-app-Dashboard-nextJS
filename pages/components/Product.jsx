...
export default function Product({
  ...
  async function createProduct(ev) {
    ...
    // check if ther are new images to be uploaded
    if (isUploading) {
      // wait for all images to be uploaded before creating the product
      await Promise.all(uploadImagesQueue);
    }

    // now u can make the API call to create or update the product
    const data = {
      title,
      description,
      price,
    ...
    // Redirect to the products page after creating or updating the product
    setRedirect(true);
  }

  async function uploadImages(ev) {
    // Get the list of selected files from the event
